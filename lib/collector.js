...
const { isFirstParty, getLocalStorage, safeJSONParse } = require('./tools');
...
async function run (uri, argv) {
  ...
  const { setup_cookie_recording } = require('./setup-cookie-recording');
  const { setup_beacon_recording } = require('./setup-beacon-recording');
  const { setup_websocket_recording } = require('./setup-websocket-recording');
  const { set_cookies } = require('./set-cookies');
