...
module.exports.set_cookies = async function (logger, setCookie, page, uri_ins) {
  // variable that will buffer all the valid cookies that are passed
  let cookieJar = [];
  if (fs.existsSync(setCookie)) {
    // passed argument is the location of a file
    logger.log(
      "info",
      `Read cookie parameter from the existing file: ${setCookie}`
    );
    ...
    const lines = fs
      .readFileSync(argv.setCookie, "UTF-8")
      .trim()
      .split(/\r?\n/);
